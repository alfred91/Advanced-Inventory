    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div class="border p-4 rounded text-center transition-transform transform hover:scale-105 hover:bg-gray-100 cursor-pointer" wire:click="showEditModal({{ $category->id }})">
            <div class="w-full h-36 overflow-hidden rounded-lg flex items-center justify-center">
                <img src="{{ asset($category->image) }}" alt="{{ $category->name }}" class="w-full h-full object-contain mb-4 mt-4">
            </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full mx-2">
            ...
            <form wire:submit.prevent="saveCategory" class="space-y-4">
                ...
                <div class="flex justify-center space-x-2 mt-4">
                    ...
                    @if ($isEdit)
                    <button wire:click="viewProducts('{{ $name }}')" class="inline-flex items-center px-4 py-2 bg-green-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none focus:border-green-900 focus:ring focus:ring-green-300 disabled:opacity-25 transition">
                        Ver Productos
                    </button>
                    @endif
                </div>
